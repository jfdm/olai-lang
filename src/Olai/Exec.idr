...
throw : Running.Error -> Olai a
throw = (throw . Exec)

panic : (why : String) -> Olai a
panic = (throw . Panic)

error : (why : FileError) -> Olai a
error = (throw . Outside)
  ...
  namespace Stmt
    ...
    return : {store,store' : List Ty.Base}
          ...
          -> (rest : Stmt.Result stack' store' b)
                  -> Olai (Stmt.Result stack' store b)
...
namespace Heap
  namespace Expr
    ...
    insert : {store : List Ty.Base}
          ...
          -> (value : Value store type)
          -> (heap  : Heap  store)
                   -> Olai (Expr.Result store (REF type))
  ...
  namespace Stmt
    ...
    join : {type : Ty.Base}
        ...
        -> (stmt  : Stmt roles types stack stack' type)
                 -> Olai (Stmt.Result stack' store type)
    ...
    eval : {type : Ty.Base}
        ...
        -> (stmt  : Stmt roles types stack out type)
                 -> Olai (Stmt.Result out store type)
