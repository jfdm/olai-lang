...
import public Olai.Lexer.Token
import Olai.Core
...
tokenMap = with List
  ...
  , (Olai.Lexer.charLit, (LitChr . stripQuotes))
  ]
  ++
...
namespace Olai
  ...
  IsKeyword s = Elem s Olai.Keywords

  public export
  IsSymbol : String -> Type
  IsSymbol s = Elem s Olai.Symbols

  export
  Lexer : Lexer Token
  Lexer = MkLexer (Olai.Lexer.tokenMap) keep EndInput

  export
  lexFile : String -> Olai (List (WithBounds Token))
  lexFile fname
